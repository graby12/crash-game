...
export default function CrashGame({ showControls = true }) {
  ...
  const fetchLiveUsers = async () => {
    try {
      const res = await fetch("http://localhost:5000/api/live-users");
  ...
  const startRound = async () => {
    try {
      const res = await fetch("http://localhost:5000/api/crash");
      const data = await res.json();
  ...
  const handlePlaceBet = async () => {
    ...
    try {
      ...
      const res = await fetch("http://localhost:5000/api/bet/bet", {
        method: "POST",
        headers: {
  ...
  const handleWithdraw = async () => {
    if (running && betPlaced && !cashedOut) {
      try {
        const token = localStorage.getItem("token");
        const res = await fetch("http://localhost:5000/api/bet/withdraw", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
